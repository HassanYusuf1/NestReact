...
import { NotefetchCommentById, deleteCommentNote } from './CommentServiceNote';
...
const DeleteCommentPage: React.FC = () => {
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        try {
          const fetchedComment = await NotefetchCommentById(Number(id));
  ...
  const handleDelete = async () => {
    if (!window.confirm('Are you sure you want to delete this comment for the note?')) {
      return;
    }

    try {
      await deleteCommentNote(Number(id));
      navigate('/notes'); // Navigerer tilbake til oversikten etter sletting
    } catch (error) {
